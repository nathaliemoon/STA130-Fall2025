...
Suppose we have access to data on ALL Starbucks menu items ever created (the complete population), and assume that the true population regression line to predict calories from protein content (in grams) is:
...
### (d) Using the `Q2c_processors` tibble, plot the association between `log_transistors` and `year`, faceted by `unit_type` and use `geom_smooth(se=FALSE, method="lm")` to add a line of best fit to both plots; save this combined plot in the R object `Q4d_plot`. Describe this association in each plot.
